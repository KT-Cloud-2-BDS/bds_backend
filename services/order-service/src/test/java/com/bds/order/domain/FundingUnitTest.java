...
class FundingUnitTest {
    ...
    class IsFundingPeriodTest {
        ...
        void 현재가_펀딩시작_이전이면_false_반환() {
            Funding funding = Funding.of(null, null, null, null, null, NOW.plusDays(1), NOW.plusDays(30), null, 0, null, null, null, null, null);
            assertThat(funding.isFundingPeriod(NOW)).isFalse();
        }

        @Test
        void 현재가_펀딩종료_이후이면_false_반환() {
            Funding funding = Funding.of(null, null, null, null, null, NOW.minusDays(30), NOW.minusDays(1), null, 0, null, null, null, null, null);
            assertThat(funding.isFundingPeriod(NOW)).isFalse();
        }

        @Test
        void 현재가_펀딩기간_내이면_true_반환() {
            Funding funding = Funding.of(null, null, null, null, null, NOW.minusDays(10), NOW.plusDays(10), null, 0, null, null, null, null, null);
    ...
    class CreateTest {
        ...
        void 정적_팩토리_메서드로_생성_시_기본값이_설정된다() {
            Funding funding = Funding.create("테스트 펀딩", 100L, 1000000L,
                    NOW, NOW.plusDays(30), NOW.plusDays(31), null);
            ...
            assertThat(funding.getIsSuccess()).isFalse();
            assertThat(funding.getType()).isEqualTo(FundingType.INSTANT);
        }
    }
}
